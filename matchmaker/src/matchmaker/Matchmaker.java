...
public class Matchmaker implements AutoCloseable
{
    ...
    private ObjectId runMatch(ObjectId botId1, ObjectId botId2)
    {
        ...
        List<String> inputAllLines = Collections.<String>emptyList();
        List<String> errorAllLines = Collections.<String>emptyList();
        try
        {
            ...
            inputAllLines = inGobbler.readAllLines();
            errorAllLines = errGobbler.readAllLines();

            if (inputAllLines.size() > 0)
            {
                matchId = new ObjectId(inputAllLines.get(inputAllLines.size() - 1));
            }
        }
        catch (Exception err)
        {
            ...
            System.err.print(inputAllLines);
            System.err.println("Error output of engine was:");
            System.err.print(errorAllLines);
        }
        return matchId;
    }
